...
def drive_rows(sheet, sheet_id, sample_range, data: list):
    ...
    for i in range(len(drive)):
        if len(drive[i]) < 100:
            drive[i] += [' '] * 100
    for group in data:
        local = [group[0], group[1], 'к-сть дз ->'] + [' '] * 100
        is_in = False
        in_index = None
        for j, i in enumerate(drive):
            if group[0] == i[0]:
                is_in = True
                in_index = j
                local = i
    ...
    sheet.values().batchUpdate(spreadsheetId=sheet_id, body={
        ...
        "data": [
            {
                "range": f"{sample_range}!A2:{last_column}{len(drive) + 1}",
                "majorDimension": "ROWS",
                "values": drive
            }
        ]
